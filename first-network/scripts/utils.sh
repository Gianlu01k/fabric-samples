...
#    if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
#      set -x
#   peer chaincode invoke -o orderer.master.com:7050 -C $CHANNEL_NAME -n fabprod $PEER_CONN_PARMS  -c '{"function":"createProd","Args":["PROD0"]}' >&log.txt
#      res=$?
#      set +x
#    else
#      set -x
#     #  peer chaincode invoke -o orderer.master.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc $PEER_CONN_PARMS ${INIT_ARG} -c ${CCARGS} >&log.txt
#          peer chaincode invoke -o orderer.master.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n fabprod $PEER_CONN_PARMS -c '{"function":"editProduct","Args":["PROD0", ]}' >&log.txt
