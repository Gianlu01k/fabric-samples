...
set -x
peer channel fetch newest $CHANNEL_NAME.block -o orderer.master.com:7050 -c $CHANNEL_NAME --tls --cafile $ORDERER_CA >&log.txt
res=$?
set +x
cat log.txt
verifyResult $res "Fetching config block from orderer has Failed"

joinChannelWithRetry 1 $N_ORG $DOMAIN
